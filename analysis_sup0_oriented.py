...
data = pd.read_csv("Datas/23June2021/mahdi_0_2_task.csv")

ors = [-4, -3, -2, -1, 0, 1, 2, 3, 4]
...
for i in range(len(ors)):
    for j in range(len(data)):
        if data["Orientation"][j] == ors[i] and data["SubjectAns"][j] == 'r' and data['AdaptorOrientation'][j] == 20:
            r_to_total_pos[i] += 1
        if data["Orientation"][j] == ors[i] and data["SubjectAns"][j] == 'r' and data['AdaptorOrientation'][j] == -20:
            r_to_total_neg[i] += 1
        if data["Orientation"][j] == ors[i] and data["SubjectAns"][j] == 'r' and data['AdaptorOrientation'][j] == 0:
            r_to_total_nuc[i] += 1
        if data["Orientation"][j] == ors[i] and data['AdaptorOrientation'][j] == 20:
            counter_pos[i]    += 1
        if data["Orientation"][j] == ors[i] and data['AdaptorOrientation'][j] == -20:
            counter_neg[i]    += 1
